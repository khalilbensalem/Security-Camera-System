...
void CycleStats::recordCycle(
    std::chrono::steady_clock::duration cycleDuration) {
  ++_cycleCount;
...
void CycleStats::report() const {
  ...
  double avgMs = 0.0;
  if (_cycleCount > 0) {
    avgMs = std::chrono::duration<double, std::milli>(_accumulatedDuration)
                .count() /
            _cycleCount;
  }
  ...
  std::cout << std::put_time(std::localtime(&nowTime), "%Y-%m-%d %H:%M:%S")
            ...
            << "Duree moyenne d'un cycle: " << std::fixed
            << std::setprecision(2) << avgMs << " ms" << std::endl;
}
